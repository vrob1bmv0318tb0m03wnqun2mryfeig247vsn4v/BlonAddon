...
public class PistolModule extends Module {
    ...
    private void shoot() {
        ...
        if (isSummoning || CoreUpdater.isSingleRunning()) return;
        ...
        if (mc.crosshairTarget != null && mc.crosshairTarget.getType() == HitResult.Type.ENTITY) {
            result = mc.crosshairTarget;
             ...
             result = ProjectileUtil.raycast(
                ...
                entity -> entity instanceof PlayerEntity && entity != mc.player,
                range.get() * range.get()
            );
             if (result == null) {
                 result = mc.player.raycast(range.get(), 0f, false);
             }
        }
    ...
    private void positionGun() {
        ...
        if (isSummoning) {
            if (CoreUpdater.isSingleRunning()) {
